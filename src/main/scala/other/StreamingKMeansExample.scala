...
object StreamingKMeansExample {
  ...
  def main(args: Array[String]) {
    ...
    // $example on$
    val conf = new SparkConf().setAppName("StreamingKMeansExample").setMaster("local[4]")
    val ssc = new StreamingContext(conf, Seconds(5))
    val trainingDir = "hdfs://master:9000/train"
    val testDir = "hdfs://master:9000/test"

    val trainingData: DStream[linalg.Vector] = ssc.textFileStream(trainingDir)
      .map(Vectors.parse)
    ...
    model.predictOn(testData.map(_.features)).print()
    model.predictOnValues(testData.map(lp => (lp.features, lp.features))).print()
